...
import { project, ProjectClicks } from "@/utils/schema";
  ...
  const OnProjectDelete = (projectId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const hasClicks = await db
          .select()
          .from(ProjectClicks)
          .where(eq(ProjectClicks.projectRef, projectId));

        if (hasClicks.length > 0) {
          await db
            .delete(ProjectClicks)
            .where(eq(ProjectClicks.projectRef, projectId));
        }
                    ...
                    >
                      ...
                      <div className="flex flex-col w-full">
                        <div className="flex items-center gap-3 ">
                          <label
                            htmlFor={"project-file-input" + project.id}
                            ...
                            <div className="flex gap-3 mt-3">
                              <Link2
                                className={`h-12 w-12 p-3 
                                             ...
                                             ${
                                               selectedOption ==
                                                 "link" + index && "bg-gray-600"
                                             }
                                             `}
                                onClick={() =>
                                  setSelectedOption("link" + index)
                                }
                              />
                              ...
                              <LineChart
                                className={`h-12 w-12 p-3 
                                             ...
                                             `}
                                onClick={() =>
                                  setSelectedOption("linechart" + index)
                                }
                              />
                            </div>
                          ...
                          {selectedOption == "link" + index ? (
                            ...
                            <div className="mt-2">
                              ...
                              <label
                                ...
                                htmlFor={"project-banner-file-input" + index}
                              >
                                <TwicPicture
                                  src={project?.banner}
                              ...
                              <input
                                type="file"
                                id={"project-banner-file-input" + index}
                                onChange={(event) =>
                                  handleFileUploadForProject(
                                    event,
                                    project.id,
